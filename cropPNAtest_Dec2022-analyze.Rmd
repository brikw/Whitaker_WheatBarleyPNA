...
* Small problem with contamination in this run.
* Would need a more advanced de-contamination protocol for this sequencing run than normal. (*SKIPPING FOR NOW*)
...
SbyE <- SbyE[order(SbyE$trtID),]
#identical(rownames(SbyE), rownames(SbyS)) #must be TRUE
SbyE$Diversity <- diversity(SbyS, index = 'shannon')
SbyE2 <- SbyE %>% filter(Crop.Type != 'Debris') %>% filter(trtID != 'PCRneg-1')
...
SbyS2 <- SbyS[rownames(SbyS) %in% rownames(SbyE2),]
SbyS2 <- SbyS2[,colSums(SbyS2) >0]    #31 ASVs unique to debris
dim(SbyS); dim(SbyS2)
